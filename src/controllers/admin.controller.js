...
export const getById = async (req, res) => {
    try {
        ...
        if (!mongoose.Types.ObjectId.isValid(id)) {
            return res.status(400).json({ success: false, msg: 'ID de administrador inválido' });
        }
...
export const create = async (req, res) => {
    try {
        ...
        return res.status(201).json({ success: true, msg: messages.ADMIN_CREATED, data: admin });
    } catch (error) {
        console.error(error);
        const status = error.statusCode || 500;
        const msg = error.message || messages.SERVER_ERROR;
...
export const update = async (req, res) => {
    try {
        ...
        if (id === req.admin._id.toString()) {
            return res.status(400).json({ success: false, msg: messages.ADMIN_CANNOT_EDIT_SELF });
        }
        const admin = await updateAdmin(id, req.body);
        return res.status(200).json({ success: true, msg: messages.ADMIN_UPDATED, data: admin });
    } catch (error) {
        console.error(error);
        const status = error.statusCode || 500;
        const msg = error.message || messages.SERVER_ERROR;
...
export const updatePassword = async (req, res) => {
    try {
        ...
        return res.status(200).json({ success: true, msg: messages.PASSWORD_CHANGED });
    } catch (error) {
        console.error(error);
        const status = error.statusCode || 500;
        const msg = error.message || messages.SERVER_ERROR;
